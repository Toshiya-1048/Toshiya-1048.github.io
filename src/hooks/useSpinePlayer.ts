...
import { AssetInfo, SpinePlayerOptions, SpinePlayerInstance } from '../types';
import { SPINE_CONFIG } from '../constants';
import { createError, logError } from '../utils/error';

export const useSpinePlayer = (
  containerRef: RefObject<HTMLDivElement>,
  assetInfo: AssetInfo
) => {
  const createPlayerOptions = useCallback((): SpinePlayerOptions => ({
  ...
  const initializePlayer = useCallback(() => {
    if (!containerRef.current || !window.spine) return null;

    try {
      return new window.spine.SpinePlayer(
  ...
  const cleanupPlayer = useCallback((player: SpinePlayerInstance | null) => {
    if (player) {
      try {
        player.dispose();
      } catch (err) {
